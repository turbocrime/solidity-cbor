...
library CBORDecoder {
    ...
    function readUInt256(
        ...
        if (maj == MajTag) {
            require(
                value == TagTypeBigNum,
                "invalid tag (expected TagTypeBigNum)"
            );
            ...
            require(
                maj == MajByteString,
                "invalid maj (expected MajByteString)"
            );
    ...
    function readUInt8(
        bytes memory cborData,
        uint byteIdx
    ) internal pure returns (uint8, uint) {
        uint8 maj;
        uint value;
    ...
    function readInt64(
        bytes memory cborData,
        uint byteIdx
    ) internal pure returns (int64, uint) {
        uint8 maj;
        uint value;
    ...
    function readInt32(
        bytes memory cborData,
        uint byteIdx
    ) internal pure returns (int32, uint) {
        uint8 maj;
        uint value;
    ...
    function readInt8(
        bytes memory cborData,
        uint byteIdx
    ) internal pure returns (int8, uint) {
        uint8 maj;
        uint value;
